...
def test1_cmp():
    assert compare_classification(torch.clone(tsr1), torch.clone(tsr_cpy), top_k=configs.TOP_K_MAX, logger=None) == 0, " [-] expected 0 errors"

def test2_cmp():
    tsr_false = torch.clone(tsr1)
    tsr_false[1, 0] = 2
    res: int = compare_classification(torch.clone(tsr_false), torch.clone(tsr_cpy), top_k=configs.TOP_K_MAX, logger=None)
